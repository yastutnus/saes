		...
		try {
			if (!opponent.bot) {
				...
				if (!verification) {
					...
					return message.channel.send(`<a:no1:814445405604675616> Düello kabul edilmedi <a:no1:814445405604675616> ...`);
				}
			}
			let userHP = 500; 
			let oppoHP = 500;
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				<:amen:814445474240921610>	**${message.author.username}**: ${userHP} :heartpulse:
				<:amen:814445474240921610>	**${opponent.username}**: ${oppoHP} :heartpulse:
					`);
					const filter = res =>
